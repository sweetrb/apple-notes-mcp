...
| `delete-note` | Remove a note (moves to Recently Deleted); refuses a note already there unless `permanent: true`; `guardNoteId` + `expectedGuardContentHash` retire an original only while its verified copy is intact |
