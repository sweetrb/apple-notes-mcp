...
1. **Title Matching**: Note operations require exact title matches. If a note is not found, suggest using `search-notes` first.
...
5. **Password-Protected Notes**: These cannot be accessed via this skill.
6. **macOS Only**: This skill only works on macOS systems.

## Error Handling

- **"Note not found"**: Use search-notes to find similar titles
- **"Permission denied"**: The user needs to grant automation permission in System Settings
