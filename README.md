...
- `tags` (native tags in body order) and `attachments`, each with a `kind`
  classified from its UTI (`image`, `scan`, `drawing`, `pdf`, `audio`, `video`,
  `url`, `table`, `gallery`, `map`, `file`), its body position, and
  `previewPath` when Notes stored a preview rendition. Gallery items and
...
- `deepLink`, `isShared` (the note or any enclosing folder is shared),
  `isLocked`, `isPinned`, `lastViewed`, `checklistTotal`, `checklistDone`,
  `hasDrawing` (classic sketches and Paper drawings), and `firstImage` (the
  first image in body order, else the first scan or drawing).
...
HTML. Errors carry a stable code in brackets, such as `[not-found]` or
`[no-full-disk-access]`.
